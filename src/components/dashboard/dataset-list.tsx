...
export function DatasetList({ datasets }: { datasets: Dataset[] | null }) {
  ...
  if (datasets.length === 0) {
    return (
      <p className="mt-4 rounded-xl border border-dashed border-rule-strong bg-paper/60 px-5 py-8 text-center text-sm text-ink-dim">
  ...
  return (
    <ul className="mt-4 space-y-2">
      {datasets.map((d) => (
        <li
          key={d.id}
          className="rounded-xl border border-rule bg-paper p-4 shadow-sm shadow-ink/[0.03]"
        >
          <div className="flex items-start justify-between gap-4">
            ...
            <span className="shrink-0 rounded-full border border-rule bg-paper-raised/60 px-2.5 py-1 font-mono text-[0.625rem] uppercase tracking-[0.1em] text-ink-faint">
              No receipt yet
            </span>
          </div>
          {d.description && (
